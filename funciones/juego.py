...
def manejar_envido_completo(puntos_jugador: int, puntos_maquina: int, mano_jugador: list, mano_maquina: list, turno_actual: str, seleccion : int) -> tuple:
    """
    Maneja el flujo completo del Envido.
    ...
    opciones_envido = ["envido",]
    valores_envido = {"envido": 2}
    valores_rechazados = {"envido": 1}
    ...
    while not envido_terminado:
        if turno_actual == "jugador":
            print("Envido")
            canto_jugador = input("Cantas envido:").strip().lower()
...
def evaluar_mano(cartas: tuple, valores_truco: dict) -> str:
    ...
    valor_jugador = valores_truco[f"{carta_jugador[0]} {carta_jugador[1]}"]
    valor_maquina = valores_truco[f"{carta_maquina[0]} {carta_maquina[1]}"]
...
def reiniciar_ronda(mazo: list, mano_jugador: list, mano_maquina: list, cartas_jugadas: list,
    ...
    mazo.clear()
    nuevo_mazo, rutas_imagenes, valores_truco = crear_mazo(PALOS,VALORES)
